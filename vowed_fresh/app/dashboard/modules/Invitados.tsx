  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        <input value={newName} onChange={e => setNewName(e.target.value)} onKeyDown={e => e.key === 'Enter' && add()} placeholder="Nombre del invitado..." style={{ flex: 1, border: '1px solid #DCE7F4', borderRadius: 12, padding: '11px 16px', fontSize: 13, outline: 'none' }} />
      ...
      {guests.length === 0 ? (
        ...
        <div style={{ border: '1px solid #EEF2F7', borderRadius: 16, overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1.5fr 0.8fr 1fr 1fr auto', padding: '10px 18px', background: '#FAFBFD', borderBottom: '1px solid #EEF2F7' }}>
            {['NOMBRE', 'CONTACTO', 'MESA', 'MENÚ', 'ASISTENCIA', ''].map(h => (
              <span key={h} style={{ fontSize: 10, color: MUTE, letterSpacing: '0.04em' }}>{h}</span>
            ))}
          </div>
          {guests.map((g, i) => (
            <div key={g.id} style={{ display: 'grid', gridTemplateColumns: '1.5fr 1.5fr 0.8fr 1fr 1fr auto', alignItems: 'center', padding: '10px 18px', borderBottom: i < guests.length - 1 ? '1px solid #F0F3F8' : 'none' }}>
              ...
              <select value={g.rsvp} onChange={e => update(g.id, 'rsvp', e.target.value)} style={{ border: '1px solid #EEF2F7', borderRadius: 8, padding: '5px 8px', fontSize: 11, color: INK, background: 'white', outline: 'none' }}>
                <option>Pendiente</option><option>Sí</option><option>No</option>
              </select>
              <button onClick={() => remove(g.id)} style={{ background: 'none', border: 'none', color: '#C7D2E0', cursor: 'pointer', fontSize: 16, padding: '0 4px' }}>×</button>
            </div>
          ))}
        </div>
      )}
