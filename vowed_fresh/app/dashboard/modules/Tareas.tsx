    ...
    <div>
      ...
      {tasks.length === 0 ? (
        ...
        <div style={{ border: '1px solid #EEF2F7', borderRadius: 16, overflow: 'hidden' }}>
          {tasks.map((t, i) => (
            <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 18px', borderBottom: i < tasks.length - 1 ? '1px solid #F0F3F8' : 'none' }}>
              <span
                onClick={() => toggle(t.id)}
                style={{ width: 18, height: 18, borderRadius: 6, flexShrink: 0, border: t.done ? 'none' : '1.5px solid #DCE7F4', background: t.done ? BLUE : 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
              >
              ...
              <span style={{ flex: 1, fontSize: 13, color: t.done ? '#C7D2E0' : INK, textDecoration: t.done ? 'line-through' : 'none' }}>{t.title}</span>
              <button onClick={() => remove(t.id)} style={{ background: 'none', border: 'none', color: '#C7D2E0', cursor: 'pointer', fontSize: 16, padding: '0 4px' }}>×</button>
            </div>
          ))}
        </div>
      )}
