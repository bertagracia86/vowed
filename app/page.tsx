    ...
    <main style={{ fontFamily: "'Inter',sans-serif", overflowX: 'hidden', background: '#f7fbff' }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        html,body{background:#52c5fa;padding:5px;}
        ...
        .btn-blue{background:${BLUE};color:white;border:none;border-radius:999px;padding:14px 36px;font-size:15px;font-weight:600;cursor:pointer;text-decoration:none;display:inline-block;transition:all 0.2s;box-shadow:0 4px 20px rgba(82,197,250,0.5);}
        .btn-blue:hover{transform:translateY(-1px);box-shadow:0 8px 32px rgba(82,197,250,0.7);}
        ...
        .feat-c{background:white;border:1px solid rgba(82,197,250,0.2);border-radius:20px;padding:32px;transition:all 0.3s;}
        .feat-c:hover{border-color:rgba(82,197,250,0.5);transform:translateY(-4px);box-shadow:0 16px 48px rgba(82,197,250,0.12);}
        ...
        .logo-img{filter:hue-rotate(-15deg) saturate(1.3) brightness(1.05);}
      `}</style>
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 300, background: scrollY > 20 ? 'rgba(255,255,255,0.98)' : 'white', borderBottom: '1px solid rgba(0,0,0,0.06)', padding: '0 48px', height: 68, display: 'flex', alignItems: 'center', justifyContent: 'space-between', transition: 'all 0.3s', backdropFilter: 'blur(12px)' }}>
        <Link href="/"><img src="/logo.png" alt="mylov3" className="logo-img" style={{ height: 26, display: 'block' }} /></Link>
      ...
      {/* TICKER */}
      <div style={{ overflow: 'hidden', borderTop: '1px solid rgba(82,197,250,0.2)', borderBottom: '1px solid rgba(82,197,250,0.2)', padding: '18px 0', background: 'white' }}>
        <div className="ticker">
          {['Invitados','·','Presupuesto','·','Mesas','·','Cronograma','·','Proveedores','·','Web de boda','·','Notas','·','Inspiración','·','Invitados','·','Presupuesto','·','Mesas','·','Cronograma','·','Proveedores','·','Web de boda','·','Notas','·','Inspiración','·'].map((t, i) => (
            <span key={i} style={{ fontSize: 12, color: t === '·' ? 'rgba(82,197,250,0.4)' : '#aac4d8', letterSpacing: '0.1em', textTransform: 'uppercase', whiteSpace: 'nowrap', flexShrink: 0 }}>{t}</span>
          ))}
        </div>
      </div>
      ...
      {/* DASHBOARD DEMO */}
      <section style={{ padding: '60px 40px 120px', background: '#eef9ff' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div className="dash-wrap floating" style={{ background: 'white', borderRadius: 20, overflow: 'hidden', boxShadow: '0 40px 120px rgba(82,197,250,0.15)', border: '1px solid rgba(82,197,250,0.2)' }}>
            <div style={{ background: '#f0faff', borderBottom: '1px solid rgba(82,197,250,0.15)', padding: '12px 20px', display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ display: 'flex', gap: 6 }}>
                {['#ff5f57','#febc2e','#28c840'].map(c => <div key={c} style={{ width: 10, height: 10, borderRadius: '50%', background: c }} />)}
              </div>
              <div style={{ flex: 1, background: 'rgba(82,197,250,0.1)', borderRadius: 6, padding: '4px 12px', fontSize: 11, color: '#7ab8d0', textAlign: 'center' }}>mylov3.app/dashboard</div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '180px 1fr', minHeight: 400 }}>
              <div style={{ background: '#f0faff', borderRight: '1px solid rgba(82,197,250,0.15)', padding: '20px 12px' }}>
                <img src="/logo.png" alt="" className="logo-img" style={{ height: 18, marginBottom: 28, marginLeft: 8 }} />
                {[{icon:'⊞',l:'Resumen',a:true},{icon:'☑',l:'Tareas'},{icon:'€',l:'Presupuesto'},{icon:'♡',l:'Invitados'},{icon:'◉',l:'Mesas'},{icon:'📅',l:'Cronograma'}].map(n => (
                  <div key={n.l} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderRadius: 8, marginBottom: 2, background: n.a ? 'rgba(82,197,250,0.15)' : 'transparent', cursor: 'pointer' }}>
              ...
              <div style={{ padding: 20, background: 'white' }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 16 }}>
                  {[{l:'Presupuesto',v:'17.300 €',s:'3.500 € restantes'},{l:'Tareas',v:'2/7',s:'5 pendientes'},{l:'Invitados',v:'48',s:'32 confirmados'},{l:'Días',v:'127',s:'para el gran día'}].map(c => (
                    <div key={c.l} style={{ background: '#f0faff', border: '1px solid rgba(82,197,250,0.2)', borderRadius: 12, padding: '12px 14px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  <div style={{ background: '#f0faff', border: '1px solid rgba(82,197,250,0.15)', borderRadius: 12, padding: 14 }}>
                    ...
                    {[{t:'Reservar la finca',done:true},{t:'Contratar fotógrafo',done:true},{t:'Enviar invitaciones',done:false},{t:'Elegir el menú',done:false},{t:'Prueba del vestido',done:false}].map(t => (
                      <div key={t.t} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                        <div style={{ width: 14, height: 14, borderRadius: 4, flexShrink: 0, background: t.done ? BLUE : 'transparent', border: t.done ? 'none' : '1px solid rgba(82,197,250,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  <div style={{ background: '#f0faff', border: '1px solid rgba(82,197,250,0.15)', borderRadius: 12, padding: 14 }}>
                    ...
                    {[{cat:'Catering',est:8000,paid:3000},{cat:'Finca',est:5000,paid:500},{cat:'Fotografía',est:2800,paid:2800},{cat:'Flores',est:1500,paid:0}].map(b => (
                      <div key={b.cat} style={{ marginBottom: 10 }}>
                        ...
                        <div style={{ height: 3, background: 'rgba(82,197,250,0.15)', borderRadius: 2 }}>
      ...
      <section style={{ position: 'relative', height: '55vh', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(238,249,255,0.15)' }} />
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div className="sec-reveal" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
            {[
              ...
              <div key={r.n} style={{ background: '#f0faff', border: '1px solid rgba(82,197,250,0.2)', borderRadius: 20, padding: 28 }}>
      ...
      {/* CTA FINAL */}
      <section style={{ padding: '120px 40px', background: '#eef9ff', textAlign: 'center' }}>
      ...
      {/* FOOTER */}
      <footer style={{ borderTop: '1px solid rgba(82,197,250,0.2)', padding: '32px 48px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'white' }}>
        <img src="/logo.png" alt="mylov3" className="logo-img" style={{ height: 22, display: 'block' }} />
