  ...
  useEffect(() => {
    ...
    const frag = `
      ...
      void main(){
        ...
        vec3 a=vec3(.04,.07,.16),b=vec3(.08,.14,.28),cc=vec3(.55,.78,.97),d=vec3(1.);
...
function DashboardMockup() {
  ...
  return (
    <div style={{background:'#0d1224',borderRadius:20,overflow:'hidden',boxShadow:'0 40px 120px rgba(0,0,0,0.6)',border:'1px solid rgba(142,197,247,0.15)'}}>
      {/* Topbar */}
      <div style={{background:'#111827',borderBottom:'1px solid rgba(255,255,255,0.06)',padding:'12px 20px',display:'flex',alignItems:'center',gap:12}}>
        <div style={{display:'flex',gap:6}}>
          {['#ff5f57','#febc2e','#28c840'].map(c=><div key={c} style={{width:10,height:10,borderRadius:'50%',background:c}}/>)}
        </div>
        <div style={{flex:1,background:'rgba(255,255,255,0.05)',borderRadius:6,padding:'4px 12px',fontSize:11,color:'rgba(255,255,255,0.3)',textAlign:'center'}}>mylov3.app/dashboard</div>
      </div>
      {/* Dashboard layout */}
      <div style={{display:'grid',gridTemplateColumns:'180px 1fr',minHeight:420}}>
        {/* Sidebar */}
        <div style={{background:'#0d1224',borderRight:'1px solid rgba(255,255,255,0.05)',padding:'20px 12px'}}>
          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:28,padding:'0 8px'}}>
            <img src="/logo.png" alt="" style={{height:18,filter:'brightness(0) invert(1)',opacity:.8}}/>
          </div>
          {[
            ...
            <div key={n.l} style={{display:'flex',alignItems:'center',gap:10,padding:'8px 10px',borderRadius:8,marginBottom:2,background:n.a?'rgba(142,197,247,0.12)':'transparent',cursor:'pointer'}}>
              <span style={{fontSize:13,color:n.a?BLUE:'rgba(255,255,255,0.35)'}}>{n.icon}</span>
              <span style={{fontSize:12,color:n.a?BLUE:'rgba(255,255,255,0.35)',fontWeight:n.a?600:400}}>{n.l}</span>
            </div>
          ))}
        </div>
        {/* Main */}
        <div style={{padding:20,background:'#0f1628'}}>
          {/* Stats row */}
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:16}}>
            {[
              ...
              <div key={c.l} style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:12,padding:'12px 14px'}}>
                <p style={{fontSize:10,color:'rgba(255,255,255,0.35)',marginBottom:6}}>{c.l}</p>
                <p style={{fontSize:18,fontWeight:600,color:'white',marginBottom:2}}>{c.v}</p>
                <p style={{fontSize:10,color:'rgba(142,197,247,0.7)'}}>{c.s}</p>
              </div>
            ))}
          </div>
          {/* Two columns */}
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
            ...
            <div style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:12,padding:14}}>
              <p style={{fontSize:11,color:'rgba(255,255,255,0.4)',marginBottom:12}}>Próximas tareas</p>
              {tasks.map(t=>(
                <div key={t.t} style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
                  <div style={{width:14,height:14,borderRadius:4,flexShrink:0,background:t.done?BLUE:'transparent',border:t.done?'none':'1px solid rgba(255,255,255,0.2)',display:'flex',alignItems:'center',justifyContent:'center'}}>
                    {t.done&&<span style={{fontSize:8,color:'white'}}>✓</span>}
                  </div>
                  <span style={{fontSize:11,color:t.done?'rgba(255,255,255,0.25)':'rgba(255,255,255,0.7)',textDecoration:t.done?'line-through':'none'}}>{t.t}</span>
                </div>
              ))}
            </div>
            {/* Budget */}
            <div style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:12,padding:14}}>
              <p style={{fontSize:11,color:'rgba(255,255,255,0.4)',marginBottom:12}}>Presupuesto</p>
              {budget.map(b=>(
                <div key={b.cat} style={{marginBottom:10}}>
                  <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
                    <span style={{fontSize:11,color:'rgba(255,255,255,0.6)'}}>{b.cat}</span>
                    <span style={{fontSize:10,color:'rgba(255,255,255,0.3)'}}>{b.paid.toLocaleString('es-ES')} / {b.est.toLocaleString('es-ES')} €</span>
                  </div>
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    s1.onload=()=>{
      ...
      s2.onload=()=>{
        ...
        // Hero text parallax
        gsap.to('.hero-title',{yPercent:-25,ease:'none',scrollTrigger:{trigger:'.hero-sec',start:'top top',end:'bottom top',scrub:true}})
        gsap.to('.hero-sub',{yPercent:-15,ease:'none',scrollTrigger:{trigger:'.hero-sec',start:'top top',end:'bottom top',scrub:true}})
        ...
        gsap.to('.dash-wrap',{rotateX:0,ease:'none',scrollTrigger:{trigger:'.dash-section',start:'top bottom',end:'center center',scrub:true}})
  ...
  const navBg = scrollY > 60 ? 'rgba(255,255,255,0.95)' : 'transparent'
  const navBorder = scrollY > 60 ? '1px solid rgba(0,0,0,0.06)' : '1px solid transparent'
  const navColor = scrollY > 60 ? INK : 'white'

  return (
    <main style={{fontFamily:"'Inter',sans-serif",overflowX:'hidden',background:INK}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .nav-a{font-size:13px;text-decoration:none;transition:opacity 0.2s;opacity:0.7;}
        ...
        .cta-ghost{background:transparent;color:rgba(255,255,255,0.6);border:1px solid rgba(255,255,255,0.2);border-radius:999px;padding:16px 40px;font-size:15px;font-weight:500;cursor:pointer;text-decoration:none;display:inline-block;transition:all 0.2s;}
        .cta-ghost:hover{border-color:rgba(255,255,255,0.5);color:white;}

        @keyframes ticker{from{transform:translateX(0)}to{transform:translateX(-50%)}}
        .ticker{display:flex;gap:48px;animation:ticker 24s linear infinite;width:max-content;align-items:center;}

        .feat-c{background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:20px;padding:32px;transition:background 0.3s,border 0.3s;}
        .feat-c:hover{background:rgba(142,197,247,0.06);border-color:rgba(142,197,247,0.25);}

        .dash-wrap{transform-origin:center top;perspective:1000px;}

        @keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-12px)}}
        .floating{animation:float 6s ease-in-out infinite;}

        @keyframes pulse{0%,100%{opacity:1}50%{opacity:0.4}}
        .live-dot{width:7px;height:7px;border-radius:50%;background:#4ade80;animation:pulse 2s ease-in-out infinite;}
      `}</style>
      ...
      <nav style={{position:'fixed',top:17,left:17,right:17,zIndex:200,backdropFilter:'blur(20px)',WebkitBackdropFilter:'blur(20px)',background:navBg,border:navBorder,borderRadius:16,padding:'14px 24px',display:'flex',alignItems:'center',justifyContent:'space-between',transition:'all 0.4s'}}>
        <Link href="/"><img src="/logo.png" alt="mylov3" style={{height:24,display:'block',filter:scrollY>60?'none':'brightness(0) invert(1)'}}/></Link>
        <div style={{display:'flex',gap:32}}>
          {['Funciones','Precios','Inspiración'].map(l=>(
            <a key={l} href="#" className="nav-a" style={{color:navColor}}>{l}</a>
          ))}
        </div>
      ...
      {/* ── HERO ── */}
      <section className="hero-sec" ref={heroRef} style={{position:'relative',minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',overflow:'hidden',background:INK}}>
        <FluidCanvas/>
        <div style={{position:'relative',zIndex:10,textAlign:'center',padding:'0 24px',maxWidth:1000,margin:'0 auto'}}>
          <div style={{display:'inline-flex',alignItems:'center',gap:8,background:'rgba(142,197,247,0.1)',border:'1px solid rgba(142,197,247,0.2)',borderRadius:999,padding:'6px 16px',marginBottom:40}}>
            <div className="live-dot"/>
            <span style={{fontSize:12,color:BLUE,fontWeight:500}}>+2.400 parejas ya lo usan · Gratis</span>
          </div>
          <h1 className="hero-title" style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'clamp(3.5rem,9vw,7rem)',fontWeight:400,lineHeight:1.0,color:'white',marginBottom:32,letterSpacing:'-0.03em'}}>
            El lugar donde<br/><em style={{color:BLUE}}>vuestra boda</em><br/>cobra vida
          </h1>
          <p className="hero-sub" style={{fontSize:17,color:'rgba(255,255,255,0.55)',maxWidth:480,margin:'0 auto 48px',lineHeight:1.8}}>
        ...
        <div style={{position:'absolute',bottom:32,left:'50%',transform:'translateX(-50%)',display:'flex',flexDirection:'column',alignItems:'center',gap:8,opacity:0.4}}>
          <span style={{fontSize:11,color:'white',letterSpacing:'0.12em',textTransform:'uppercase'}}>Scroll</span>
          <div style={{width:1,height:40,background:'white',opacity:0.5}}/>
        </div>
      </section>

      {/* ── TICKER ── */}
      <div style={{overflow:'hidden',borderTop:'1px solid rgba(255,255,255,0.05)',borderBottom:'1px solid rgba(255,255,255,0.05)',padding:'18px 0',background:'rgba(255,255,255,0.02)'}}>
        <div className="ticker">
          {['Invitados','·','Presupuesto','·','Mesas','·','Cronograma','·','Proveedores','·','Web de boda','·','Notas','·','Inspiración','·','Invitados','·','Presupuesto','·','Mesas','·','Cronograma','·','Proveedores','·','Web de boda','·','Notas','·','Inspiración','·'].map((t,i)=>(
            <span key={i} style={{fontSize:12,color:t==='·'?'rgba(255,255,255,0.15)':'rgba(255,255,255,0.4)',letterSpacing:'0.1em',textTransform:'uppercase',whiteSpace:'nowrap',flexShrink:0}}>{t}</span>
          ))}
        </div>
      </div>

      {/* ── FEATURES ── */}
      <section style={{padding:'120px 40px',background:INK}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <div className="sec-reveal" style={{marginBottom:72,display:'grid',gridTemplateColumns:'1fr 1fr',gap:40,alignItems:'end'}}>
            <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'clamp(2rem,4vw,3.5rem)',fontWeight:400,color:'white',lineHeight:1.1,letterSpacing:'-0.02em'}}>
              Todo en un<br/>solo lugar
            </h2>
            <p style={{fontSize:15,color:'rgba(255,255,255,0.45)',lineHeight:1.85,paddingBottom:4}}>
          ...
          <div className="feats" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:16}}>
            {[
              ...
              <div key={f.t} className="feat-c">
                ...
                <p style={{fontFamily:"'Cormorant Garamond',serif",fontSize:22,color:'white',marginBottom:10,fontWeight:400}}>{f.t}</p>
                <p style={{fontSize:13,color:'rgba(255,255,255,0.4)',lineHeight:1.75}}>{f.d}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── DASHBOARD DEMO ── */}
      <section id="demo" className="dash-section" style={{padding:'60px 40px 120px',background:INK}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
      ...
      <section style={{position:'relative',height:'60vh',overflow:'hidden',display:'flex',alignItems:'center',justifyContent:'center'}}>
        <img src="https://images.unsplash.com/photo-1605985687770-2e2e82c9b5f1?w=1800&q=80" alt="" style={{position:'absolute',inset:0,width:'100%',height:'130%',objectFit:'cover',marginTop:'-15%',filter:'brightness(0.35)'}}/>
        <div className="sec-reveal" style={{position:'relative',zIndex:2,textAlign:'center',padding:'0 40px'}}>
          <p style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'clamp(2rem,5vw,4.5rem)',fontWeight:400,fontStyle:'italic',color:'white',lineHeight:1.2,marginBottom:32}}>
      ...
      {/* ── SOCIAL PROOF ── */}
      <section style={{padding:'120px 40px',background:'#080d1a'}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <div className="sec-reveal" style={{textAlign:'center',marginBottom:72}}>
            <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'clamp(2rem,4vw,3.5rem)',fontWeight:400,color:'white',lineHeight:1.1,letterSpacing:'-0.02em',marginBottom:16}}>
              Parejas que ya lo vivieron
            </h2>
            <p style={{fontSize:14,color:'rgba(255,255,255,0.35)'}}>+2.400 bodas planificadas con mylov3</p>
          </div>
          <div className="sec-reveal" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:16}}>
            {[
              ...
              <div key={r.n} style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:20,padding:28}}>
                <div style={{display:'flex',gap:3,marginBottom:18}}>
                  {[...Array(5)].map((_,i)=><span key={i} style={{color:BLUE,fontSize:14}}>★</span>)}
                </div>
                <p style={{fontSize:14,color:'rgba(255,255,255,0.65)',lineHeight:1.8,marginBottom:24,fontStyle:'italic'}}>"{r.q}"</p>
                <div>
                  <p style={{fontSize:13,fontWeight:600,color:'white'}}>{r.n}</p>
                  <p style={{fontSize:11,color:'rgba(255,255,255,0.3)'}}>{r.d}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ── CTA FINAL ── */}
      <section style={{padding:'120px 40px',background:INK,textAlign:'center'}}>
        <div className="sec-reveal" style={{maxWidth:700,margin:'0 auto'}}>
          <div style={{fontSize:32,color:BLUE,marginBottom:24}}>♡</div>
          <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'clamp(2.5rem,5vw,5rem)',fontWeight:400,color:'white',lineHeight:1.05,letterSpacing:'-0.03em',marginBottom:20}}>
            Vuestra historia de amor merece el mejor comienzo
          </h2>
          <p style={{fontSize:15,color:'rgba(255,255,255,0.4)',marginBottom:48,lineHeight:1.8}}>Gratis para siempre. Sin tarjeta de crédito.</p>
          <Link href="/dashboard" className="cta-btn-lg">Empezar a planificar gratis</Link>
        </div>
      </section>

      {/* ── FOOTER ── */}
      <footer style={{borderTop:'1px solid rgba(255,255,255,0.05)',padding:'32px 48px',display:'flex',alignItems:'center',justifyContent:'space-between',background:'#080d1a'}}>
        <img src="/logo.png" alt="mylov3" style={{height:22,display:'block',filter:'brightness(0) invert(1)',opacity:0.6}}/>
        <p style={{fontSize:12,color:'rgba(255,255,255,0.2)'}}>Hecho con ♡ para parejas que quieren disfrutar del proceso</p>
        <p style={{fontSize:11,color:'rgba(255,255,255,0.15)'}}>2025</p>
      </footer>
    </main>
  )
}
