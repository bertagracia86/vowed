...
interface Props { guests: Guest[]; setGuests: (g: Guest[]) => void; onNavigate?: (t: string) => void; readOnly?: boolean }
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(36,28,23,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'white', borderRadius: 18, width: 460, maxHeight: '85vh', overflowY: 'auto', padding: '26px 28px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <p style={{ fontFamily: F, fontSize: 22, color: TEXT_PRIMARY }}>Añadir invitados</p>
        ...
        {tab === 'info' ? (
          <>
            <div style={{ background: '#F7F4EF', borderRadius: 12, padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 20 }}>
              <span style={{ fontSize: 12, color: TEXT_PRIMARY, lineHeight: 1.4 }}>¿No tenéis todos los datos de vuestros invitados?</span>
            ...
            <p style={{ fontSize: 13, fontWeight: 600, color: TEXT_PRIMARY, marginBottom: 10 }}>Invitado principal</p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
              <div>
                <label style={{ fontSize: 11, color: TEXT_SECONDARY }}>Nombre*</label>
              ...
              <div>
                <label style={{ fontSize: 11, color: TEXT_SECONDARY }}>Apellidos*</label>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
              <div>
                <label style={{ fontSize: 11, color: TEXT_SECONDARY }}>Email</label>
              ...
              <div>
                <label style={{ fontSize: 11, color: TEXT_SECONDARY }}>Móvil</label>
            ...
            <p style={{ fontSize: 12, fontWeight: 600, color: TEXT_PRIMARY, marginBottom: 8 }}>¿Invitado?</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {(['Definitivamente', 'Quizás'] as const).map(opt => (
                <label key={opt} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: TEXT_PRIMARY, cursor: 'pointer' }}>
          ...
          <>
            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 11, color: TEXT_SECONDARY }}>Nombre en el sobre</label>
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 11, color: TEXT_SECONDARY }}>Dirección</label>
            ...
            <div>
              <label style={{ fontSize: 11, color: TEXT_SECONDARY }}>Ciudad</label>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 24, paddingTop: 16, borderTop: '1px solid #ECE9E4' }}>
          ...
          <button onClick={save} style={{ background: firstName.trim() ? '#898a76' : '#DCD4C8', color: 'white', border: 'none', borderRadius: 999, padding: '9px 20px', fontSize: 12, fontWeight: 600, cursor: firstName.trim() ? 'pointer' : 'default' }}>Guardar</button>
        </div>
      </div>
    </div>
  )
...
function RsvpDonut({ confirmed, pending, declined }: { confirmed: number; pending: number; declined: number }) {
  ...
  return (
    <svg width="120" height="120" viewBox="0 0 120 120">
      ...
      <circle cx="60" cy="60" r={r} fill="none" stroke="GREEN" strokeWidth="11" strokeDasharray={`${cSeg} ${circ - cSeg}`} strokeDashoffset={0} transform="rotate(-90 60 60)" strokeLinecap="round" />
...
export default function Invitados({ guests, setGuests: setGuestsRaw, onNavigate, readOnly }: Props) {
  const setGuests = readOnly ? () => {} : setGuestsRaw
  ...
  return (
    <div>
      <h1 style={{ fontFamily: F, fontSize: 26, fontWeight: 500, color: TEXT_PRIMARY, marginBottom: 16 }}>Invitados y RSVP</h1>
      ...
      <div style={{ display: 'flex', gap: 20, borderBottom: '1px solid #ECE9E4', marginBottom: 14, overflowX: 'auto' }}>
        {TABS.map(t => (
          <button key={t} onClick={() => selectTab(t)} style={{
            ...
            color: tab === t ? INK : MUTE, fontWeight: tab === t ? 600 : 500, borderBottom: tab === t ? '2px solid #898a76' : '2px solid transparent'
          }}>{t}</button>
        ))}
      </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
          {SUBTABS.map(t => (
            <button key={t} onClick={() => setSubtab(t)} style={{
              ...
              padding: '8px 14px', fontSize: 12.5, color: subtab === t ? '#6b4226' : MUTE, fontWeight: subtab === t ? 600 : 500
            }}>{t}</button>
          ))}
        </div>
      )}
      ...
      {tab === 'Recopilar contactos' && (
        <div>
          ...
          <p style={{ fontSize: 11, color: TEXT_SECONDARY, marginBottom: 10 }}>Pendientes de completar ({guests.filter(g => !g.contact || !g.address).length})</p>
          <div style={{ border: '1px solid #F5EFE0', borderRadius: 16, overflow: 'hidden' }}>
            {guests.filter(g => !g.contact || !g.address).length === 0 ? (
              <p style={{ fontSize: 13, color: TEXT_SECONDARY, textAlign: 'center', padding: '30px 0' }}>Todos vuestros invitados tienen sus datos completos ♡</p>
      ...
      {(tab === 'Eventos' || subtab === 'Invitar a eventos') && tab !== 'Recopilar contactos' && tab !== 'Agradecimientos' && tab !== 'Bloques de hotel' && tab !== 'Lista de invitados' && (
        <div style={{ border: '1px solid #F5EFE0', borderRadius: 16, overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: `1.5fr repeat(${EVENTS.length}, 1fr)`, padding: '10px 18px', background: '#FBF9F5', borderBottom: '1px solid #F5EFE0' }}>
            <span style={{ fontSize: 10, color: TEXT_SECONDARY, letterSpacing: '0.01em' }}>INVITADO</span>
            {EVENTS.map(ev => <span key={ev} style={{ fontSize: 10, color: TEXT_SECONDARY, letterSpacing: '0.01em', textAlign: 'center' }}>{ev}</span>)}
          </div>
          {guests.map((g, i) => (
      ...
      {tab === 'Agradecimientos' && (
        <div style={{ border: '1px solid #F5EFE0', borderRadius: 16, overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1.5fr auto', padding: '10px 18px', background: '#FBF9F5', borderBottom: '1px solid #F5EFE0' }}>
            <span style={{ fontSize: 10, color: TEXT_SECONDARY, letterSpacing: '0.01em' }}>INVITADO</span>
            <span style={{ fontSize: 10, color: TEXT_SECONDARY, letterSpacing: '0.01em' }}>ESTADO</span>
            <span style={{ fontSize: 10, color: TEXT_SECONDARY, letterSpacing: '0.01em' }}>AGRADECIDO</span>
          </div>
          {guests.length === 0 ? (
            <p style={{ fontSize: 13, color: TEXT_SECONDARY, textAlign: 'center', padding: '30px 0' }}>Sin invitados todavía.</p>
          ) : guests.map((g, i) => (
      ...
      {tab === 'Bloques de hotel' && (
        <div>
          <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
            ...
            <button onClick={addBlock} style={{ background: BLUE, color: 'white', border: 'none', borderRadius: 12, padding: '11px 22px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Añadir</button>
          </div>
          {blocks.length === 0 ? (
            <p style={{ fontSize: 13, color: TEXT_SECONDARY, textAlign: 'center', padding: '30px 0' }}>Sin bloques de hotel todavía.</p>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {blocks.map(b => (
                <div key={b.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'white', border: '1px solid #ECE9E4', borderRadius: 14, padding: '14px 18px' }}>
                  <div>
                    <p style={{ fontSize: 14, fontWeight: 500, color: TEXT_PRIMARY }}>{b.hotel}</p>
                    <p style={{ fontSize: 11, color: TEXT_SECONDARY }}>Código: {b.code || '—'} · {b.rooms} habitaciones reservadas</p>
                  </div>
      ...
      {tab === 'Lista de invitados' && subtab === 'Direcciones de sobres' && (
        <div style={{ border: '1px solid #F5EFE0', borderRadius: 16, overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 2fr', padding: '10px 18px', background: '#FBF9F5', borderBottom: '1px solid #F5EFE0' }}>
            <span style={{ fontSize: 10, color: TEXT_SECONDARY, letterSpacing: '0.01em' }}>INVITADO</span>
            <span style={{ fontSize: 10, color: TEXT_SECONDARY, letterSpacing: '0.01em' }}>DIRECCIÓN POSTAL</span>
          </div>
          {guests.map((g, i) => (
            <div key={g.id} style={{ display: 'grid', gridTemplateColumns: '1.5fr 2fr', alignItems: 'center', padding: '10px 18px', borderBottom: i < guests.length - 1 ? '1px solid #F5EFE0' : 'none' }}>
              <span style={{ fontSize: 13, color: TEXT_PRIMARY }}>{g.name}</span>
              <input value={g.address} onChange={e => update(g.id, 'address', e.target.value)} placeholder="Calle, ciudad, código postal..." style={{ border: 'none', background: 'transparent', fontSize: 12, color: TEXT_SECONDARY, outline: 'none' }} />
            </div>
          ))}
        </div>
      )}
          ...
          {(['Sí', 'Pendiente', 'No'] as const).map(status => {
            ...
            return (
              <div key={status}>
                ...
                {list.length === 0 ? (
                  <p style={{ fontSize: 12, color: TEXT_SECONDARY, paddingLeft: 4 }}>Nadie en este grupo todavía.</p>
                ) : (
                  <div style={{ border: '1px solid #F5EFE0', borderRadius: 14, overflow: 'hidden' }}>
                    {list.map((g, i) => (
                      <div key={g.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 18px', borderBottom: i < list.length - 1 ? '1px solid #F5EFE0' : 'none' }}>
                        <span style={{ fontSize: 13, color: TEXT_PRIMARY }}>{g.name}</span>
                        <span style={{ fontSize: 12, color: TEXT_SECONDARY }}>{g.contact || 'Sin contacto'}</span>
                      </div>
                    ))}
                  </div>
                )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 20 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', gap: 8, position: 'relative' }}>
              <button onClick={() => setShowFilter(s => !s)} style={{ border: activeFilters ? `1px solid #898a76` : '1px solid #ECE9E4', background: activeFilters ? '#F4E7D8' : 'white', borderRadius: 999, padding: '9px 16px', fontSize: 12, color: TEXT_PRIMARY, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              {showFilter && (
                <div style={{ position: 'absolute', top: '115%', left: 0, background: 'white', border: '1px solid #ECE9E4', borderRadius: 12, boxShadow: '0 8px 20px rgba(0,0,0,0.08)', padding: 14, zIndex: 30, width: 220 }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12.5, color: TEXT_PRIMARY, marginBottom: 10, cursor: 'pointer' }}>
                    <input type="checkbox" checked={filterInvited} onChange={e => setFilterInvited(e.target.checked)} />
                    Definitivamente invitados
                  </label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12.5, color: TEXT_PRIMARY, cursor: 'pointer' }}>
                    <input type="checkbox" checked={filterMissing} onChange={e => setFilterMissing(e.target.checked)} />
                    Con contacto pendiente
                  </label>
                </div>
              )}
              <div style={{ border: '1px solid #ECE9E4', background: 'white', borderRadius: 999, padding: '9px 16px', fontSize: 12, color: TEXT_SECONDARY, display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Buscar por nombre" style={{ border: 'none', outline: 'none', fontSize: 12, color: TEXT_PRIMARY, background: 'transparent', fontFamily: F, width: 130 }} />
              </div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <button onClick={() => setShowModal(true)} style={{ background: '#898a76', color: 'white', border: 'none', borderRadius: 999, padding: '9px 20px', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>Añadir invitados</button>
            </div>
          </div>

          {visibleGuests.length === 0 ? (
            <p style={{ fontSize: 13, color: TEXT_SECONDARY, textAlign: 'center', padding: '40px 0' }}>{guests.length === 0 ? 'Sin invitados todavía.' : 'Ningún invitado coincide con la búsqueda o filtros.'}</p>
          ) : (
            <div style={{ border: '1px solid #F5EFE0', borderRadius: 16, overflow: 'visible' }}>
              <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '8px 14px 0' }}>
                <Tooltip text="Exporta la información de los invitados a una hoja de cálculo." align="right">
                  <button onClick={exportCsv} style={{ background: 'none', border: 'none', color: TEXT_SECONDARY, cursor: 'pointer', fontSize: 16, letterSpacing: 1 }}>⋯</button>
                </Tooltip>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '0.5fr 1.3fr 1.5fr 1.4fr auto', padding: '10px 18px', background: '#FBF9F5', borderBottom: '1px solid #F5EFE0' }}>
                <Tooltip text="Total de invitados en el grupo." align="left">
                  <span style={{ fontSize: 10, color: TEXT_SECONDARY, letterSpacing: '0.01em', cursor: 'default' }}>N.º</span>
                </Tooltip>
                <Tooltip text="Ordena por apellido, ascendente o descendente." align="left">
                  <span style={{ fontSize: 10, color: TEXT_SECONDARY, letterSpacing: '0.01em', cursor: 'default' }}>NOMBRE ↑</span>
                </Tooltip>
              ...
              {visibleGuests.map((g, i) => (
                <div key={g.id} style={{ display: 'grid', gridTemplateColumns: '0.5fr 1.3fr 1.5fr 1.4fr auto', alignItems: 'center', padding: '12px 18px', borderBottom: i < visibleGuests.length - 1 ? '1px solid #F5EFE0' : 'none' }}>
                  <span style={{ fontSize: 12, color: TEXT_SECONDARY }}>{i + 1}</span>
                  <span style={{ fontSize: 13, color: TEXT_PRIMARY }}>{g.name}</span>
                  {g.contact ? (
                    <input value={g.contact} onChange={e => update(g.id, 'contact', e.target.value)} placeholder="Email o teléfono" style={{ border: 'none', background: 'transparent', fontSize: 12, color: TEXT_SECONDARY, outline: 'none' }} />
                  ) : (
                    <div>
                  ...
                  <div style={{ display: 'flex', gap: 14 }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: TEXT_PRIMARY, cursor: 'pointer' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, alignSelf: 'flex-start' }}>
          <div style={{ background: CARD, border: `1px solid ${BEIGE}`, borderRadius: 16, padding: 18 }}>
            <p style={{ fontFamily: F, fontSize: 15, color: TEXT_PRIMARY, marginBottom: 12 }}>Progreso de RSVP</p>
          ...
          <div style={{ background: CARD, border: `1px solid ${BEIGE}`, borderRadius: 16, padding: 18 }}>
            <p style={{ fontFamily: F, fontSize: 15, color: TEXT_PRIMARY, marginBottom: 12 }}>Grupos de invitados</p>
            {(() => {
                ...
                <>
                  {groups.map(gr => (
                    <div key={gr} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 0', borderBottom: `1px solid ${BEIGE}` }}>
                      <span style={{ fontSize: 12.5, color: TEXT_PRIMARY }}>{gr}</span>
          ...
          <div style={{ background: 'linear-gradient(135deg, #F4EFE8, #EFE6F5)', border: `1px solid ${BEIGE}`, borderRadius: 16, padding: 18 }}>
            <p style={{ fontFamily: F, fontSize: 15, color: TEXT_PRIMARY, marginBottom: 6 }}>Sugerencias</p>
            ...
            <button onClick={() => onNavigate && onNavigate('mensajes')} style={{ background: BROWN, color: 'white', border: 'none', borderRadius: 999, padding: '9px 18px', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
              Enviar recordatorio
            </button>
          </div>
        </div>
