...
export default function LunasMiel() {
  return (
    <div>
      <h1 style={{ fontFamily: F, fontSize: 26, fontWeight: 500, color: TEXT_PRIMARY, marginBottom: 4 }}>Lunas de miel</h1>
      <p style={{ fontSize: 12, color: TEXT_SECONDARY, marginBottom: 24 }}>Inspiración y ofertas para vuestro viaje de luna de miel.</p>

      <div style={{ background: '#F7F4EF', borderRadius: 16, padding: '28px 24px', textAlign: 'center', marginBottom: 28 }}>
        <p style={{ fontFamily: F, fontSize: 20, color: TEXT_PRIMARY, marginBottom: 8 }}>¿A dónde os apetece escaparos?</p>
        <div style={{ display: 'flex', gap: 8, justifyContent: 'center', maxWidth: 480, margin: '0 auto' }}>
          ...
          <button style={{ background: '#898a76', color: 'white', border: 'none', borderRadius: 999, padding: '11px 22px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Buscar</button>
        </div>
      </div>

      <p style={{ fontFamily: F, fontSize: 18, color: TEXT_PRIMARY, marginBottom: 12 }}>Destinos destacados</p>
        ...
        {DESTINOS.map(d => (
          <div key={d.name} style={{ borderRadius: 14, overflow: 'hidden', border: '1px solid #ECE9E4', background: 'white', cursor: 'pointer' }}>
            ...
            <div style={{ padding: '12px 14px' }}>
              <p style={{ fontSize: 13, fontWeight: 600, color: TEXT_PRIMARY, marginBottom: 2 }}>{d.name}</p>
              <p style={{ fontSize: 11, color: TEXT_SECONDARY }}>{d.price}</p>
            </div>
          </div>
        ))}
      </div>
