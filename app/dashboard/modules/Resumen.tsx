...
function Pill({ icon, children }: { icon: string; children: React.ReactNode }) {
  return (
    <span style={{ border: '1px solid #ECE9E4', borderRadius: 999, padding: '4px 10px 4px 8px', fontSize: 10, color: MUTE, display: 'flex', alignItems: 'center', gap: 4 }}>
    ...
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: 20 }}>
        ...
        <div style={{ background: 'white', border: '1px solid #ECE9E4', borderRadius: 16, padding: 14 }}>
          ...
          <p style={{ fontFamily: F, fontSize: 20, fontWeight: 600, color: INK, textAlign: 'center', marginBottom: 4 }}>
          ...
          <p style={{ fontSize: 10, color: MUTE, textAlign: 'center', marginTop: 6 }}>Consigue la app iOS</p>
        </div>
        ...
        <div style={{ background: 'white', border: '1px solid #ECE9E4', borderRadius: 16, padding: '4px 20px' }}>
          ...
          <Section title="Invitaciones y papelería" open={!!openSections['invitaciones']} onToggle={() => toggle('invitaciones')}>
            <p onClick={() => setTab('invitaciones')} style={{ fontSize: 11.5, color: MUTE, margin: '6px 0 10px', cursor: 'pointer' }}>Diseños listos para personalizar y enviar.</p>
          </Section>

          <Section title="Regalos" open={!!openSections['regalos']} onToggle={() => toggle('regalos')}>
            <p onClick={() => setTab('regalos')} style={{ fontSize: 11.5, color: MUTE, margin: '6px 0 10px', cursor: 'pointer' }}>Cread vuestra lista y compartidla con los invitados.</p>
          </Section>
          ...
          <Section title="Invitados y RSVP" open={!!openSections['invitados']} onToggle={() => toggle('invitados')}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8, marginBottom: 4, gap: 16 }}>
              ...
              <div style={{ textAlign: 'center', flexShrink: 0 }}>
                <p style={{ fontFamily: F, fontSize: 26, color: INK, fontWeight: 600 }}>{guests.length}</p>
                <p style={{ fontSize: 10, color: MUTE }}>Invitados</p>
              </div>
            </div>
          </Section>

          <Section title="Presupuesto" open={!!openSections['presupuesto']} onToggle={() => toggle('presupuesto')}>
            <p onClick={() => setTab('presupuesto')} style={{ fontSize: 11.5, color: MUTE, margin: '6px 0 10px', cursor: 'pointer' }}>
              {budgetPaid.toLocaleString('es-ES')} € pagados de {budgetEst.toLocaleString('es-ES')} €
            </p>
          </Section>

          <p onClick={() => setTab('mesas')} style={{ fontSize: 10.5, color: MUTE, textAlign: 'right', padding: '8px 0 2px', cursor: 'pointer' }}>
            ≡ Reordenar
          </p>
        </div>
      </div>
      ...
      <div style={{ marginTop: 18, background: 'white', border: '1px solid #ECE9E4', borderRadius: 16, padding: 16, display: 'grid', gridTemplateColumns: '220px repeat(4, 1fr)', gap: 12, alignItems: 'stretch' }}>
        <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          ...
          <button style={{ alignSelf: 'flex-start', border: '1px solid #ECE9E4', background: 'white', color: INK, borderRadius: 8, padding: '7px 14px', fontSize: 11, cursor: 'pointer' }}>
            Ver portfolio
          </button>
        </div>
