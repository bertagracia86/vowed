...
export default function Cronograma({ milestones, setMilestones, weddingDate, setWeddingDate }: Props) {
  ...
  return (
    <div>
      ...
      <div style={{ background: `linear-gradient(135deg, #EAF1FA, #F3F7FC)`, borderRadius: 16, padding: '28px 32px', marginBottom: 24 }}>
        ...
        <input
          ...
          style={{ border: '1px solid #DCE7F4', borderRadius: 10, padding: '10px 14px', fontSize: 14, outline: 'none', marginBottom: 16, background: 'white' }}
        />
        {daysLeft !== null ? (
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        <input value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder="Hito (ej: Enviar invitaciones)" style={{ flex: 1, border: '1px solid #DCE7F4', borderRadius: 12, padding: '11px 16px', fontSize: 13, outline: 'none' }} />
        <input type="date" value={newDate} onChange={e => setNewDate(e.target.value)} style={{ border: '1px solid #DCE7F4', borderRadius: 12, padding: '11px 14px', fontSize: 13, outline: 'none' }} />
      ...
      {sorted.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
          {sorted.map((m) => {
            ...
            return (
              <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px', background: 'white', border: '1px solid #EEF2F7', borderRadius: 12, marginBottom: 6 }}>
                <span onClick={() => toggle(m.id)} style={{ width: 18, height: 18, borderRadius: 6, flexShrink: 0, border: m.done ? 'none' : '1.5px solid #DCE7F4', background: m.done ? BLUE : 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: 13, color: m.done ? '#C7D2E0' : INK, textDecoration: m.done ? 'line-through' : 'none' }}>{m.title}</p>
                ...
                <button onClick={() => remove(m.id)} style={{ background: 'none', border: 'none', color: '#C7D2E0', cursor: 'pointer', fontSize: 16 }}>×</button>
              </div>
            )
          })}
        </div>
