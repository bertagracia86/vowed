    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr 300px', gap: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          {SECTIONS.map(s => (
            <button key={s.id} onClick={() => setSection(s.id)} style={{
              ...
              fontWeight: section === s.id ? 600 : 500, fontSize: 12.5, cursor: 'pointer'
            }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          {section === 'perfil' && (
            <div style={{ background: CARD, border: `1px solid ${BEIGE}`, borderRadius: 16, padding: 22 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
                <p style={{ fontFamily: F, fontSize: 17, color: TEXT_PRIMARY }}>Información del perfil</p>
              ...
              <div style={{ display: 'flex', gap: 20, alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <div>
                    ...
                    <p style={{ fontSize: 13.5, color: TEXT_PRIMARY }}>{weddingInfo.partner1 || 'Vosotros'} &amp; {weddingInfo.partner2 || 'dos'}</p>
                  </div>
                  <div>
                    <p style={{ fontSize: 10.5, color: SUBTEXT }}>EMAIL</p>
                    <p style={{ fontSize: 13.5, color: TEXT_PRIMARY }}>bertagracia86@gmail.com</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          {section === 'boda' && (
            <div style={{ background: CARD, border: `1px solid ${BEIGE}`, borderRadius: 16, padding: 22 }}>
              <p style={{ fontFamily: F, fontSize: 17, color: TEXT_PRIMARY, marginBottom: 18 }}>Detalles de la boda</p>
          ...
          {section === 'notif' && (
            <div style={{ background: CARD, border: `1px solid ${BEIGE}`, borderRadius: 16, padding: 22 }}>
              <p style={{ fontFamily: F, fontSize: 17, color: TEXT_PRIMARY, marginBottom: 4 }}>Preferencias de notificaciones</p>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr repeat(3, 60px)', gap: 8, alignItems: 'center' }}>
                ...
                {NOTIF_ROWS.map(row => (
                  <>
                    <span key={row.key} style={{ fontSize: 12.5, color: TEXT_PRIMARY, padding: '8px 0' }}>{row.label}</span>
          ...
          {!['perfil', 'boda', 'notif'].includes(section) && (
            <div style={{ background: CARD, border: `1px solid ${BEIGE}`, borderRadius: 16, padding: '40px 24px', textAlign: 'center' }}>
              <p style={{ fontFamily: F, fontSize: 17, color: TEXT_PRIMARY, marginBottom: 6 }}>{SECTIONS.find(s => s.id === section)?.label}</p>
              <p style={{ fontSize: 12, color: SUBTEXT }}>Muy pronto podréis gestionar esto desde aquí.</p>
            </div>
          )}

          <button onClick={onLogout} style={{ alignSelf: 'flex-start', background: 'white', border: `1px solid ${BEIGE}`, borderRadius: 999, padding: '10px 22px', fontSize: 13, color: TEXT_PRIMARY, cursor: 'pointer' }}>
            Cerrar sesión
          </button>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, alignSelf: 'flex-start' }}>
          <div style={{ background: CARD, border: `1px solid ${BEIGE}`, borderRadius: 16, padding: 18 }}>
            <p style={{ fontFamily: F, fontSize: 15, color: TEXT_PRIMARY, marginBottom: 4 }}>Vuestro plan</p>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '10px 0' }}>
              ...
              <div>
                <p style={{ fontSize: 13, fontWeight: 600, color: TEXT_PRIMARY }}>Plan gratuito</p>
          ...
          <div style={{ background: CARD, border: `1px solid ${BEIGE}`, borderRadius: 16, padding: 18 }}>
            <p style={{ fontFamily: F, fontSize: 15, color: TEXT_PRIMARY, marginBottom: 12 }}>Equipo y colaboradores</p>
            {TEAM.map(m => (
              <div key={m.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 0' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 12, color: TEXT_PRIMARY }}>{m.name}{m.you ? ' (tú)' : ''}</span>
                </div>
          ...
          <div style={{ background: CARD, border: `1px solid ${BEIGE}`, borderRadius: 16, padding: 18 }}>
            <p style={{ fontFamily: F, fontSize: 15, color: TEXT_PRIMARY, marginBottom: 4 }}>Seguridad</p>
            <p style={{ fontSize: 11, color: SUBTEXT, marginBottom: 12 }}>Mantened vuestra cuenta protegida.</p>
            <button style={{ width: '100%', border: `1px solid ${BEIGE}`, background: 'white', color: TEXT_PRIMARY, borderRadius: 999, padding: '8px 0', fontSize: 12, cursor: 'pointer' }}>Cambiar contraseña</button>
          </div>
        </div>
      </div>
    </div>
